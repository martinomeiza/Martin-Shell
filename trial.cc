...
/*
            fprintf(stderr, "%s: error with input\n", __FILE__);
            return 0;
#endif
            bg = 1;
        } else {
...
*/
        ...
        if (strcmp(com_tok[2], "&") == 0) { // background command coming
#ifdef DEBUG
        ...
        // Process input:
        if (strlen(real_com[1]) == 0) // no command, luser just pressed return
            continue;

        else if (strcmp(real_com[1], "exit") == 0) {
            printf("Bye\n");
            return 0;
        }

        else if (strcmp(real_com[1], "more") == 0) {
            ...
            if (bg) {  // background command, we fork a process that
                ...
                if (bgp == 0) { // child executing the command
                    int r = run_it(real_com[0], real_com[1], real_com, envp, path);
                    printf("& %s done (%d)\n", real_com[0], real_com[1], WEXITSTATUS(r));
                    if (r != 0) {
                        printf("& %s completed with a non-null exit code\n", real_com[0], real_com[1]);
                    }
                    return 0;
                }
